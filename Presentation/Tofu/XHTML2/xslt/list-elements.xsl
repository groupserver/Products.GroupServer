...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="xhtml2:label">
    <p>
      <xsl:if test="not(@class or @role)">
        <xsl:attribute name="class">xhtml2-label</xsl:attribute>
      </xsl:if>
      <xsl:call-template name="elementLink"/>
    </p>
  </xsl:template>
  ...
  <xsl:template match="xhtml2:ul">
    <div>
      <xsl:if test="not(@class or @role)">
        <xsl:attribute name="class">xhtml2-ul</xsl:attribute>
      </xsl:if>
  ...
  <xsl:template match="xhtml2:ol">
    <div>
      <xsl:if test="not(@class or @role)">
        <xsl:attribute name="class">xhtml2-ol</xsl:attribute>
      </xsl:if>
      <xsl:apply-templates select="@*"/>
      <xsl:apply-templates select="xhtml2:label"/> 
      <ol>
  ...
  <xsl:template match="xhtml2:nl">
    <div>
      <xsl:if test="not(@class or @role)">
        <xsl:attribute name="class">xhtml2-nl</xsl:attribute>
      </xsl:if>
      <xsl:apply-templates select="@*"/>
      <xsl:apply-templates select="xhtml2:label"/> 
      <ul>
  ...
  <xsl:template match="xhtml2:dl">
    <div>
      <xsl:if test="not(@class or @role)">
        <xsl:attribute name="class">xhtml2-dl</xsl:attribute>
      </xsl:if>
      <xsl:apply-templates select="@*"/>
      <xsl:apply-templates select="xhtml2:label"/> 
      <dl>
