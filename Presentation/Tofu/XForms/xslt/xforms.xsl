...
<xsl:stylesheet version="1.0"
	...
	exclude-result-prefixes="xf nav pageinfo dw dc rdf rss slash xml a admin"
>
  ...
  <xsl:template name="match-single">
    ...
    <xsl:variable name="orig"><xsl:value-of select="//data/xf:model/xf:instance/data/*[name()=$ref]/text()"/></xsl:variable>
  ...
  <xsl:template name="match-in-list">
    <xsl:param name="ref"/>
    <xsl:param name="match"/>
    <xsl:param name="orig"><xsl:value-of select="//data/xf:model/xf:instance/data/*[name()=$ref]/text()"/></xsl:param>

    <xsl:variable name="before"><xsl:value-of select="substring-before($orig, ' ')"/></xsl:variable>
    <xsl:variable name="after"><xsl:value-of select="substring-after($orig, ' ')"/></xsl:variable>
    <xsl:choose>

      <xsl:when test="$before = '' and $after = '' and starts-with($orig, $match) and string-length($orig) = string-length($match)">
        <xsl:value-of select="'1'"/>
      </xsl:when>
      <xsl:when test="starts-with($before, $match) and string-length($before) = string-length($match)">
        <xsl:value-of select="'1'"/>
      </xsl:when>
      <xsl:when test="$after != '' and not(starts-with($before, $match))">
        <xsl:call-template name="match-in-list">
          <xsl:with-param name="ref"><xsl:value-of select="$ref"/></xsl:with-param>

          <xsl:with-param name="match"><xsl:value-of select="$match"/></xsl:with-param>
          <xsl:with-param name="orig"><xsl:value-of select="$after"/></xsl:with-param>
        </xsl:call-template>
      </xsl:when>
      <xsl:otherwise>
  ...
  <xsl:template name="xf:model">
     ...
     <xsl:for-each select="//data/xf:model">
      ...
      <xsl:for-each select="xf:instance/data/*">
        <input type="hidden" name="__instance+{ancestor::xf:model/@id}+{name()}" value="{text()}"/>
      </xsl:for-each>
      <xsl:for-each select="xf:submission">
        <input type="hidden" name="__submission_action+{../@id}+{@id}" value="{@action}"/>
        <input type="hidden" name="__submission_method+{../@id}+{@id}" value="{@method}"/>
        <xsl:if test="@target">
            <input type="hidden" name="__submission_target+{../@id}+{@id}" value="{@target}"/>
        </xsl:if>
      </xsl:for-each>
     </xsl:for-each>
     </div>
  ...
  <xsl:template match="xf:submit">
    <div class="{@class}">
      <input type="submit" name="__submit+{@model}+{@submission}" onclick="submitButtonHandler(this)" class="{@class}" value="{xf:label/text()}" title="{xf:hint/text()}"/>
    </div>
  </xsl:template>
  ...
  <xsl:template match="xf:input">
    <div class="{@class}">
     ...
     <xsl:variable name="bind"><xsl:value-of select="@bind"/></xsl:variable>

     <!-- Build the label -->
      <xsl:call-template name="label"><xsl:with-param name="labelNode" select="xf:label"/></xsl:call-template>

      <input type="text" id="{@model}-{$ref}" name="{@model}+{$ref}" class="{@class}" title="{xf:hint/text()}">
         ...
         <xsl:if test="boolean(//data/xf:model[@id=$model]/xf:bind[@id=$bind]/@required)">
...
<div class="calendarcontrol">
 	 ...
 	 <a href="javascript: void(0);" onmouseover="if (timeoutId) clearTimeout(timeoutId);window.status='Show Calendar';return true;" onmouseout="if (timeoutDelay) calendarTimeout();window.status='';" onclick="if (!g_Calendar) new Calendar(new Date(), '{@model}-{$ref}-calendar', 'Presentation/Tofu/XForms/images/');g_Calendar.show(event,'{@model}-{$ref}',false); return false;">
            <img src="/Presentation/Tofu/XForms/images/calendar.gif" name="imgCalendar" width="34" height="21" border="0" alt=""/>
         </a>


      <!-- Add a DIV to contain the calendar control -->
	  <div class="calendar" id="{@model}-{$ref}-calendar" onmouseout="calendarTimeout();" onmouseover="if (timeoutId) clearTimeout(timeoutId);">&#160;</div>
    <!--
  ...
  <xsl:template match="xf:secret">
    <div class="{@class}">
      <label for="{@model}-{@ref}" class="{@class}"><xsl:value-of select="xf:label/text()"/></label>
      <input type="password" id="{@model}-{@ref}" name="{@model}+{@ref}" class="{@class}" title="{xf:hint/text()}"><xsl:attribute name="value">
  ...
  <xsl:template name="getref">
    ...
    <xsl:choose>
        <xsl:when test="$item/@bind">
            <xsl:value-of select="//data/xf:model[@id=$model]/xf:bind[@id=$item/@bind]/@nodeset"/>
        </xsl:when>
    ...
    <div class="{@class}">
     <!-- Build the label -->
      <xsl:call-template name="label"><xsl:with-param name="labelNode" select="xf:label"/></xsl:call-template>
       ...
       <textarea name="{@model}+{$ref}" class="{@class}" id="{@model}-{$ref}" title="{xf:hint/text()}">
  ...
  <xsl:template match="xf:upload">
    <div class="{@class}">
      <label for="{@model}-{@ref}" class="{@class}"><xsl:value-of select="xf:label/text()"/></label>
      <input type="file" name="{@model}+{@ref}" id="{@model}-{@ref}" title="{xf:hint/text()}" class="{@class}" value=""/>
    </div>
  </xsl:template>
  ...
  <xsl:template match="xf:select1">
    ...
    <xsl:choose>
      <xsl:when test="@appearance='minimal' or @appearance='compact'">
        <select name="{@model}+{$ref}" id="{@model}-{$ref}" title="{xf:hint/text()}" class="@class">
          <xsl:if test="boolean(//data/xf:model[@id=$model]/xf:bind[@id=$bind]/@required)">
             <xsl:attribute name="class">required</xsl:attribute>
          </xsl:if>
		  <!-- Copy across all other attributes in the xhtml namespace -->
          ...
          <xsl:variable name="selected"><xsl:call-template name="match-single">
            ...
            <option value="{xf:value/text()}"><xsl:if test="$selected='1'"><xsl:attribute name="selected">1</xsl:attribute></xsl:if><xsl:value-of select="xf:label/text()"/></option>
          </xsl:for-each>
        </select>
      ...
      <xsl:otherwise>
        <xsl:for-each select="xf:item">
          ...
          <input type="radio" title="{../xf:hint/text()}" name="{../@model}+{$ref}" id="{../@model}-{$ref}" value="{xf:value/text()}"><xsl:if test="$selected='1'"><xsl:attribute name="checked">1</xsl:attribute></xsl:if></input><label><xsl:value-of select="xf:label/text()"/></label>
  ...
  <xsl:template match="xf:select">
    ...
    <xsl:choose>
      <xsl:when test="@appearance='minimal' or @appearance='compact'">
        <select name="{$model}+{$ref}" id="{$model}-{$ref}" title="{xf:hint/text()}" multiple="1">
          <xsl:if test="boolean(//data/xf:model[@id=$model]/xf:bind[@id=$bind]/@required)">
              <xsl:attribute name="class">required</xsl:attribute>
          </xsl:if>
		  <!-- Copy across all other attributes in the xhtml namespace -->
          ...
          <xsl:for-each select="xf:item">
            <xsl:variable name="selected"><xsl:call-template name="match-in-list">
                                            <xsl:with-param name="ref" select="$ref"/>
                                            <xsl:with-param name="match" select="xf:value/text()"/>
      ...
      <xsl:otherwise>
        <xsl:for-each select="xf:item">
          ...
          <input type="checkbox" title="{../xf:hint/text()}" name="{../@model}+{$ref}" id="{../@model}-{$ref}" value="{xf:value/text()}"><xsl:if test="$selected='1'"><xsl:attribute name="checked">1</xsl:attribute></xsl:if></input>
          <label for="{../@model}-{$ref}"><xsl:value-of select="xf:label/text()"/></label>
        </xsl:for-each>
      </xsl:otherwise>
    </xsl:choose>
  ...
  <xsl:template name="label">
    ...
    <xsl:variable name="model"><xsl:value-of select="$labelParent/@model"/></xsl:variable>
    <xsl:variable name="bind"><xsl:value-of select="$labelParent/@bind"/></xsl:variable>
    <xsl:variable name="class"><xsl:value-of select="$labelParent/@class"/></xsl:variable>
