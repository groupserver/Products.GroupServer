...
<xsl:output method="html" indent="yes" encoding="UTF8" omit-xml-declaration="yes"
...
<div id="toparea">
	...
	<div id="titlebar">
		...
		<xsl:if test="output/metadata/bannerimage/@url">
                  <span class="bannerimage"><a href="{output/metadata/bannerimage/@link}"><img src="{output/metadata/bannerimage/@url}" alt="banner image"/></a></span>
                </xsl:if>
	</div>
	<!-- Title Bar Ends -->
...
<xsl:template match="output/content">
        <xsl:if test="../messages/message[@type='result']"><xsl:call-template name="result-messages"/></xsl:if>
	<xsl:apply-templates/>
</xsl:template>
